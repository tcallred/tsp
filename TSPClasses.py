...
class SubProblem:
    def __init__(self, path, parent_matrix, cost, cost_to_here, parent_lb):
        self.cost = cost
        self.path = path
        self.lb = parent_lb + cost_to_here
        self.reduced_c_matrix = self._get_reduced_cost(parent_matrix)

    def __lt__(self, other):
        return False

    def _get_reduced_cost(self, matrix):
        new_matrix = matrix.copy()
        for row in new_matrix:
            r_min = row.min()
            if r_min != 0 and r_min != math.inf:
                self.lb += r_min
                row -= r_min

        for col in new_matrix.T:
            c_min = col.min()
            if c_min != 0 and c_min != math.inf:
                self.lb += c_min
                col -= c_min
